...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(
        JSON.stringify({ error: 'Autenticação necessária', news: healthNews }),
    ...
    if (authError || !claimsData?.claims) {
      return new Response(
        JSON.stringify({ error: 'Autenticação inválida', news: healthNews }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const userId = claimsData.claims.sub;
    console.log('RSS translation requested by user:', userId);
