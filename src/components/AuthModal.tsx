...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook' | 'apple') => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao fazer login com rede social');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 flex items-center justify-between text-white">
          <div>
            <h2 className="font-bold text-xl">Entrar no TERVIS.AI</h2>
            <p className="text-sm text-blue-100 mt-1">
              Faça login para usar a busca inteligente
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <>
              <div className="space-y-3">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full flex items-center justify-center gap-3 px-4 py-3 border-2 border-gray-300 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
